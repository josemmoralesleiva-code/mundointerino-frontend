...
export default function Admin() {
  ...
  const { users, loading, setUsers, fetchAll, verifyUser } = useUsers()
  ...
  const verificar = async (id: string, estado: string, motivo = '') => {
    try {
      ...
      setUsers((prev: any) => prev.map((u: any) => u._id === id ? updated : u))
  ...
  const stats = {
    total:       users.length,
    pendientes:  users.filter(u => u.verificacionEstado === 'pendiente').length,
    verificados: users.filter(u => u.verificacionEstado === 'verificado').length,
    rechazados:  users.filter(u => u.verificacionEstado === 'rechazado').length,
  }
